...
def input_node_matrix(n):
    node_matrix = np.zeros((n, 2))
    print("Enter node matrix (format: x y):")
    for i in range(n):
        x, y = map(float, input(f"Enter coordinates for node {i + 1}: ").split())
        node_matrix[i] = [x, y]
    return node_matrix

def input_nodal_loads(n):
    nodal_loads = np.zeros((n, 2))
    print("Enter nodal loads (format: Fx Fy):")
    for i in range(n):
        fx, fy = map(float, input(f"Enter loads for node {i + 1}: ").split())
        nodal_loads[i] = [fx, fy]
    return nodal_loads

def input_constraints(n):
    constraints = np.zeros((n, 2))
    print("Enter constraints/supports conditions (format: Cx Cy):")
    for i in range(n):
        cx, cy = map(int, input(f"Enter constraints for node {i + 1}: ").split())
        constraints[i] = [cx, cy]
    return constraints

def input_adjacency_matrix(n):
    adjacency_matrix = np.zeros((n, n))
    print("Enter adjacency/connectivity matrix:")
    for i in range(n):
        print(f"For node {i + 1}, enter the connections (0 or 1):")
        connections = list(map(int, input().split()))
        adjacency_matrix[i] = connections
...
# #node_matrix = input_node_matrix(n)
# nodal_loads = input_nodal_loads(n)
# constraints = input_constraints(n)
# adjacency_matrix = input_adjacency_matrix(n)

n = 3
node_matrix = [(-16,10),(9,58),(110,12)]
constraints = [(1, 1), (0, 1), (0, 0)]
nodal_loads = [(0, 0), (10, 0), (1000,-200)]
adjacency_matrix = [
    [1, 1, 1],
    [1, 1, 1],
    [1, 1, 1],
